...
/**
 * @brief programme permettant la réalisation d'un histogramme sur une zone précise : Bd_St_Michel
 * @return
 */
int main(void) {
    ...
    */ 
    ...
    for (int i=0; i<nbPoint; i++)   
    {
      ...
      /*
       * comptage du nombre de fois que le trafic est inconnu sur Bd_St_Michel
       */ 
    int comptInconnu=0;
    for (int i=0; i<nbPoint; i++)    
    {
        if (lines[i*10 + 6] == "etatTrafic : Inconnu" && lines[i*10 + 2] == "libelle : Bd_St_Michel")  
        {
        comptInconnu++;
        }
     }
      ...
      /*
       * comptage du nombre de fois que le trafic est pré-saturé sur Bd_St_Michel
       */ 
    int comptPresat=0;
    for (int i=0; i<nbPoint; i++)    
    {

        if (lines[i*10 + 6] == "etatTrafic : Pré-saturé" && lines[i*10 + 2] == "libelle : Bd_St_Michel")  
        {
        comptPresat++;
        }
     }
      ...
      /*
       * comptage du nombre de fois que le trafic est saturé sur Bd_St_Michel
       */
    int comptSatur=0;
    for (int i=0; i<nbPoint; i++)    // nombre de fois qu'une données est etudiées
    {
     ...
     // char recupStr[3];
        if (lines[i*10 + 6] == "etatTrafic : Saturé" && lines[i*10 + 2] == "libelle : Bd_St_Michel")  // on ne prend pas les champs vides (débit horaires)
        {
        comptSatur++;
        }
     }
  ...
  int rouge, blanc, noir, bleu, marron, vert; /* Deux couleurs */
  ...
  image = gdImageCreate(1500, 1000);
    ...
    */
    string trafic ="ETAT DU TRAFIC A BD ST MICHEL - JOURNEE DU "; // titre initial (à compléter)
    string trafic2="          ";
