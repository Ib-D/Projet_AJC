...
/**
 * @brief programme permettant la réalisation d'un histogramme sur une zone précise : Av_Victor_Hugo
 * @return
 */
int main(void) {
    ...
    */ 
    ...
    for (int i=0; i<nbPoint; i++)  
    {
        if (lines[i*10 + 6] == "etatTrafic : Inconnu" && lines[i*10 + 2] == "libelle : Av_Victor_Hugo")  
        {
        comptInconnu++;
        }
     }
    ...
    {
        if (lines[i*10 + 6] == "etatTrafic : Pré-saturé" && lines[i*10 + 2] == "libelle : Av_Victor_Hugo")  
        {
        comptPresat++;
        }
     }
    ...
    for (int i=0; i<nbPoint; i++)   
    {
        if (lines[i*10 + 6] == "etatTrafic : Saturé" && lines[i*10 + 2] == "libelle : Av_Victor_Hugo")  
        {
        comptSatur++;
        }
     }
    ...
    mesPolices[4] = gdFontGiant;
      ...
      */ 
      ...
      int hSat =800 - round(comptSatur*600/3000);
        ...
        */ 
        string trafic ="ÉTAT DU TRAFIC AVENUE VICTOR HUGO - JOURNÉE DU ";  // titre initial (à compléter)
        string trafic2="          ";
